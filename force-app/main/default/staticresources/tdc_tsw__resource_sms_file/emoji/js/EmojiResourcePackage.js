...
(function($, window, document) {
  ...
  */
  ...
  var EmojiArea_WYSIWYG = function($textarea, id, options) {
    ...
    this.$editor.on("change keydown keyup resize scroll", function(e) {
      if(MAX_LENGTH_ALLOWED_KEYS.indexOf(e.which) == -1 &&
        ...
        editorDiv.text().length + (editorDiv.find('img').length)*2 >= editorDiv.attr('maxlength'))
      {
        e.preventDefault();
      }
	CharacterSengmentCount(editorDiv.text(), editorDiv.find('img').length);
    ...
    this.$editor.on("paste", function (e) {
      e.preventDefault();
      var content;
      var charsRemaining = editorDiv.attr('maxlength') - (editorDiv.text().length + (editorDiv.find('img').length)*2);
	CharacterSengmentCount(editorDiv.text(), editorDiv.find('img').length);
    ...
    $textarea.after("<i class='emoji-picker-icon emoji-picker " + this.options.popupButtonClasses + "' data-id='" + id + "' data-type='picker'><img src='/resource/resource_sms_file/emoji/favicon.png'/></i>");
  ...
  EmojiArea_WYSIWYG.prototype.insert = function(emoji) {
     ...
     */
    util.emojiInserted(emoji, this.menu);
	CharacterSengmentCount(this.$editor.text(), this.$editor.find('img').length);
    this.onChange();
  };
...
//      scrollHeight -= parseInt(this.css('padding-top')) + parseInt(this.css('padding-bottom'));
  ...
  EmojiMenu.prototype.onItemSelected = function(emoji) {
    if(this.emojiarea.$editor.text().length + (this.emojiarea.$editor.find('img').length)*2 >= this.emojiarea.$editor.attr('maxlength'))
    {
      return;
    }
	CharacterSengmentCount(this.emojiarea.$editor.text(), this.emojiarea.$editor.find('img').length);
...
/*****modification: Added by Rajeev Khare, in order to character count ***/
function CharacterSengmentCount(smstext, argcount){
		console.log(smstext);
		console.log(argcount);
		var currelmvalue = smstext;//$(".myText").val();
              var isTrue = isGSMAlphabet(currelmvalue);
              var segment = 1;
              var nonGsmLength = (argcount*2) + currelmvalue.length;
              var strLength = (argcount*2) + currelmvalue.length + count(currelmvalue,'{') + count(currelmvalue,'}') + count(currelmvalue,'[') + count(currelmvalue,']');
              if(isTrue && argcount == 0){
                   if(strLength <= 160){
                     segment = 1;
                   }
                   else{
		...
		var numberOfCharacters = nonGsmLength;
                document.getElementById('segmentId').style.color = "black";
                if (numberOfCharacters == 1) {
                    document.getElementById('segmentId').innerHTML = nonGsmLength+' Character / '+parseInt(segment,10)+' Segment';
                } else {
			if (numberOfCharacters == 0) {
		            document.getElementById('segmentId').innerHTML = 'You Can Enter Up To 1000 Characters';
		        } else {
	                	var sgmnt = parseInt(segment, 10);
	                	if(sgmnt == 1)
	                    		sgmnt = sgmnt + ' Segment';
	                   	else
	                   		sgmnt = sgmnt + ' Segments';
				document.getElementById('segmentId').innerHTML = nonGsmLength + ' Characters / ' + sgmnt;	                
				}
